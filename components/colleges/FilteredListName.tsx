    ...
    <div>
      <section className="border-b border-black/10 mb-3 px-4 pb-5">
        ...
        <div className="max-h-[165px] overflow-y-auto pr-1">
          ...
          {
            name ? (<ul>
              {filteredList.slice(0, visibleItems)?.map((list) => (
                <li key={list.id} className='flex items-center gap-3 mb-3 relative'>
                  {
                    isLoading ? <Skeleton circle width={'1.5rem'} height={'1.5rem'} /> : <input
                      type="radio"
                      name="stream"
                      value={list.name.toLowerCase()}
                      checked={list.name.toLowerCase() === selectedItem}
                      onChange={() => onFilterChange(selected, list.name.toLowerCase())}
                      className="hidden"
                      id={list.name.toLowerCase() + title}

                    />
                  }
                  {
                    isLoading ? <Skeleton width={'10rem'} /> : <label
                      htmlFor={list.name.toLowerCase() + title}
                      className="flex items-center gap-3 mb-3 cursor-pointer"
                    >
                      <span
                        className={`w-5 h-5 inline-block border-2 rounded-full ${list.name.toLowerCase() === selectedItem ? 'bg-blue-500 border-blue-500' : 'border-gray-400'
                          }`}
                      >
                        {list.name.toLowerCase() === selectedItem && (
...
export default FilteredListName