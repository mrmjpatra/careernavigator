...
const FilteredList: FC<FilterSectionProps> = ({ title, selectedItem, selected, list }) => {
                        ...
                        {filteredList.slice(0, visibleItems)?.map((item) => (
                            <li key={item} className="flex items-center gap-3 mb-3 relative">
                                <input
                                    ...
                                    checked={item?.toLowerCase() === selectedItem}
                                ...
                                <label htmlFor={item?.toLowerCase() + title} className="flex items-center gap-3 mb-3 cursor-pointer">
                                    <span
                                        className={`w-5 h-5 inline-block border-2 rounded-full ${item?.toLowerCase() === selectedItem ? 'bg-blue-500 border-blue-500' : 'border-gray-400'
                                            }`}
                                    >
                                        {item?.toLowerCase() === selectedItem && <span><TiTick color="white" /></span>}
                                    </span>
                                    {
                                        <span>{convertWord(item)}</span>
                                    }
