...
export default async function Image({ params }: { params: { name: string } }) {
  const data = await fetch(`${process.env.HOST}/api/school/name/${getFormattedString(params.name)}`).then((res) =>
    res.json()
  )
    ...
    (
      <div tw="relative flex w-full h-full flex items-center justify-center">
        ...
        <div tw="absolute flex inset-0">
          <img
            tw="flex flex-1"
            src={data.message.schoolPhoto.downloadUrl}
            alt={data.message.schoolName}
          />
          {/* Overlay */}
        ...
        <div tw="flex flex-col text-neutral-50">
          {/* Title */}
          <div tw="text-7xl font-bold text-center">{convertWord(data.message.schoolName)}</div>
          {/* Tags */}
