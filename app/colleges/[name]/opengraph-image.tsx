...
export default async function Image({ params }: { params: { name: string } }) {
  const data = await fetch(`${process.env.HOST}/api/college/name/${getFormattedString(params.name)}`, { next: { revalidate: 600000 } }).then((res) =>
    res.json()
  )
    ...
    (
      <div tw="relative flex w-full h-full flex items-center justify-center">
        ...
        <div tw="absolute flex inset-0">
          <img
            tw="flex flex-1"
            src={data.message.collegePhoto.downloadUrl}
            alt={data.message.collegeName}
          />
          {/* Overlay */}
        ...
        <div tw="flex flex-col text-neutral-50">
          {/* Title */}
          <div tw="text-7xl font-bold text-center">{convertWord(data.message.collegeName)}</div>
          {/* Tags */}
